...
class InfoPopupElement {
    ...
    public function invalidate(p_land: Land): Void {
        ...
        popupElement.getChildByName("info_popup_rarity", true).model = land.getRarity().charAt(0).toUpperCase() + land.getRarity().substr(1);
        popupElement.getChildByName("info_popup_rarity", true).skin.color = TileRarityType.getColorForRarity(land.getRarity());
