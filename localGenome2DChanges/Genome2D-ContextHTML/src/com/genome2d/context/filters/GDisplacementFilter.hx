    ...
    public function new(p_scaleX:Float = .1, p_scaleY:Float = .1) {
        ...
        fragmentCode = "
            ...
            void main(void)
            {
                ...
                copyTexCoord.y = copyTexCoord.y * 0.5;
                copyTexCoord.x = copyTexCoord.x * 0.5;
                vec4 cDisp = texture2D(sDisplacementTexture, copyTexCoord);

                copyTexCoord.x = vTexCoord.x + (cDisp.r - 0.5) * scale.x;
                copyTexCoord.y = vTexCoord.y + (cDisp.r - 0.5) * scale.y;
